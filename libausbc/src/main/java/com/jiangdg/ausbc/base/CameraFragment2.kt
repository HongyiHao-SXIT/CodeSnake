 ...
 */
abstract class CameraFragment2 : BaseFragment(), ICameraStateCallBack {
    ...
    private lateinit var rightTextureView: TextureView
    ...
    override fun initView() {
        when (val cameraView = getCameraView()) {
            ...
            getCameraViewContainer()?.apply {
                Log.d(TAG, "initView: removeAllViews "  )
                removeAllViews()
                addView(view, LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.MATCH_PARENT, 1f))
    ...
    protected fun openCamera(st: IAspectRatio? = null) {
        when (st) {
            ...
            getCurrentCamera()?.setCameraStateCallBack(this@CameraFragment2)
        }
    }
